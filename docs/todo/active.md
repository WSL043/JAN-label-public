...
| --- | --- | --- | --- | --- | --- | --- |
| T-001 | `#1` | P0 | ready | Codex | Zint CLI adapter を `crates/barcode` に実装 | 実バイナリパスを受けて render 呼び出しをテストできる |
| T-002 | `#2` | P0 | ready | Codex | `render` に PDF 出力ルートを追加 | SVG と並んで PDF fixture を比較できる |
| T-003 | `#3` | P1 | ready | Codex | `importer` の行単位バリデーションを実装 | 列だけでなくセル値エラーを返せる |
