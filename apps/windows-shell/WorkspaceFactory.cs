...
public static class WorkspaceFactory
{
    public static void SeedModules(ObservableCollection<ModuleModel> modules)
    {
        modules.Add(
            new ModuleModel(
                "Home",
                "Overview and session status",
                "overview",
                "Operational overview for label review, preview, and print readiness.",
                "The shell should surface connection status, current blockers, and the next safe action without sending the operator into another workspace first.",
                new[]
                {
                ...
                new[]
                {
                    ...
                    new RibbonGroupModel(
                        ...
                        ShellActionModel.Enabled(ShellActions.CatalogRules, "Refresh and focus catalog-governance context resolved directly in the Windows shell.")),
                    new RibbonGroupModel(
                        "Workspace",
                        ShellActionModel.Disabled(ShellActions.CurrentState, "This workspace summary is not yet wired in the native shell."),
                        ShellActionModel.Disabled(ShellActions.ReleaseNotes, "Release-note viewing is not yet wired in the native shell."),
                        ShellActionModel.Disabled(ShellActions.PreviewPackage, "Package inspection is not yet wired in the native shell.")),
                },
                new[]
                {
                    new ContextBadgeModel("Service", "local companion", Brushes.SteelBlue),
                    new ContextBadgeModel("Preview", "installer ready", Brushes.ForestGreen),
                ...
                new[]
                {
                    ...
                    new StatusStripItemModel("Preview", "installer"),
                    new StatusStripItemModel("Service", "companion-backed"),
                },
                BuildHomeWorkspace()));
        ...
        modules.Add(
            new ModuleModel(
                ...
                "Authoring is only production-safe when saved catalog state, preview output, and proof approval are easy to distinguish from the live draft at a glance.",
                new[]
                {
        ...
        modules.Add(
            new ModuleModel(
                ...
                "24",
                "Shared batch snapshot fallback",
                "This fallback keeps the batch lane readable until a live shared snapshot is available.",
                new[]
                {
                    ShellActionModel.Disabled(ShellActions.ImportWorkbook, "Batch submission remains outside the native shell in this release."),
                    ShellActionModel.Disabled(ShellActions.RetryFailed, "Retry remains outside the native shell in this release."),
                    ShellActionModel.Disabled(ShellActions.QueueSnapshot, "Shared batch snapshot refresh requires the local companion service."),
                },
                new[]
                {
                    new RibbonGroupModel(
                        "Import",
                        ShellActionModel.Disabled(ShellActions.Csv, "Batch import remains outside the native shell in this release."),
                        ShellActionModel.Disabled(ShellActions.Xlsx, "Batch import remains outside the native shell in this release."),
                        ShellActionModel.Disabled(ShellActions.AliasMap, "Batch import remains outside the native shell in this release.")),
                    new RibbonGroupModel(
                        "Queue",
                        ...
                        ShellActionModel.Disabled(ShellActions.RetryFailed, "Retry remains outside the native shell in this release."),
                        ShellActionModel.Disabled(ShellActions.FreezeRow, "Batch queue mutation remains outside the native shell in this release.")),
                    new RibbonGroupModel(
                        "Validation",
                        ShellActionModel.Disabled(ShellActions.FixtureCheck, "Batch validation remains outside the native shell in this release."),
                        ShellActionModel.Disabled(ShellActions.UnknownTemplate, "Batch validation remains outside the native shell in this release."),
                        ShellActionModel.Disabled(ShellActions.JanWarnings, "Batch validation remains outside the native shell in this release.")),
                },
                new[]
                {
                    new ContextBadgeModel("Import", "csv / xlsx", Brushes.SteelBlue),
                    new ContextBadgeModel("Snapshot", "fallback", Brushes.DarkGoldenrod),
                    new ContextBadgeModel("Queue", "preview", Brushes.ForestGreen),
                },
                new[]
                {
                    new StatusStripItemModel("Mode", "Batch review"),
                    new StatusStripItemModel("Snapshot", "fallback"),
                    new StatusStripItemModel("Service", "shared snapshot"),
    ...
    private static HomeWorkspaceModel BuildHomeWorkspace()
    {
        ...
        {
            HeaderDetail = "local overview snapshot",
            StatusSummary = "ready to review",
        ...
        model.SummaryCards.Add(new SummaryCardModel("Preview", "installer", "The packaged workstation opens directly into the WPF shell."));
        model.SummaryCards.Add(new SummaryCardModel("Service", "connected path", "Proof and audit data still come from the local service; catalog state now reads directly from packaged and local manifest files."));
        model.SummaryCards.Add(new SummaryCardModel("Next", "review", "Check preview, pending proofs, and snapshot health before dispatch work."));
        model.SessionRows.Add(new PropertyRowModel("Branch", "main"));
        model.SessionRows.Add(new PropertyRowModel("Release base", "v0.2.0"));
        model.SessionRows.Add(new PropertyRowModel("Latest preview", "windows-native-shell-main"));
        model.SessionRows.Add(new PropertyRowModel("Validation", "GitHub Windows runner authoritative"));
        model.SessionRows.Add(new PropertyRowModel("Primary shell", "JAN Label WPF workstation"));
        ...
        model.ActivityRows.Add(new ActivityRowModel("14:22", "release", "Single-package workstation build refreshed from Windows CI", "done"));
        model.ActivityRows.Add(new ActivityRowModel("14:10", "designer", "Designer frame and preview surface aligned to the workstation shell", "done"));
        model.ActivityRows.Add(new ActivityRowModel("13:58", "catalog", "Default, draft, and rollback template states are visible from one board", "done"));
        model.ActivityRows.Add(new ActivityRowModel("13:47", "history", "Backup bundle visibility stays available for recovery review", "ok"));
        model.ActivityRows.Add(new ActivityRowModel("13:35", "proof", "Approved proof lineage still gates print release", "ok"));
        model.ControlSections.Add(new PropertySectionModel("Workstation Layout", "The WPF shell is now the primary operator surface while proof and print guardrails stay unchanged.", new[] { new PropertyRowModel("Workspace style", "native Windows workstation"), new PropertyRowModel("Proof / print gate", "service-backed review"), new PropertyRowModel("Template save path", "local catalog overlay") }));
        model.ControlSections.Add(new PropertySectionModel("Core Lanes", "These workspaces cover the normal operator loop from preview to review history.", new[] { new PropertyRowModel("Print Console", "proof queue and dispatch checklist"), new PropertyRowModel("Batch Jobs", "shared queue review and blockers"), new PropertyRowModel("History", "audit search, proof review, retention visibility") }));
        model.NextSteps.Add(new QueueItemModel("Review the template board", "Confirm default, draft, and rollback state without leaving Overview", "catalog", "The operator should be able to see which template wins before entering Designer."));
        model.NextSteps.Add(new QueueItemModel("Check the packaged preview", "Confirm module switching and panel density on Windows", "preview", "Use the packaged workstation instead of loose publish output."));
        model.NextSteps.Add(new QueueItemModel("Review pending work", "Open Print Console or History to clear proof and audit blockers", "review", "Use Overview to decide where the next operator action belongs."));
        model.StatusItems.Add(new StatusItemModel("Packaging", "single installer", "The workstation is packaged as one user-facing Windows app.", "OK", Brushes.ForestGreen));
        model.StatusItems.Add(new StatusItemModel("Overview", "ready", "The shell keeps the current lanes visible in one place.", "LIVE", Brushes.SteelBlue));
        model.StatusItems.Add(new StatusItemModel("Catalog", "overlay preserved", "Packaged and local template state stay explicit.", "WATCH", Brushes.DarkGoldenrod));
        return model;
    }
    ...
    private static DesignerWorkspaceModel BuildDesignerWorkspace()
    {
        ...
        var model = new DesignerWorkspaceModel
        {
            ...
            StatusSummary = "service-backed review still controls proof and print",
        ...
        model.SetupRows.Add(new PropertyRowModel("Dispatch route", "service workflow"));
        ...
        model.StatusItems.Add(new StatusItemModel("Bridge", "connected", "service-backed proof gate", "OK", Brushes.ForestGreen));
    ...
    private static PrintConsoleWorkspaceModel BuildPrintConsoleWorkspace()
    {
        var model = new PrintConsoleWorkspaceModel
        {
            ...
            ProofQueueSummary = "seeded proof review lane",
            TimelineSummary = "seeded audit-derived timeline",
        };
        model.ProofQueue.Add(new QueueItemModel("proof-240416-017", "basic-50x30@v2 | 4901234567894", "pending", "Needs operator approval before dispatch lane unlocks.", "proof review queue", "artifact valid / lineage pending", "Pending review blocks 200-145-4 dispatch.", "Approve or reject after confirming saved overlay and subject."));
        model.ProofQueue.Add(new QueueItemModel("proof-240416-018", "shipper-70x50@v1 | 4901234567801", "approved", "Current approved lineage available for print.", "approved lineage", "artifact pinned / route open", "No blocker. Ready jobs can use this proof.", "Use as the approved reference before dispatch."));
        model.ProofQueue.Add(new QueueItemModel("proof-240416-019", "basic-50x30@v2 | 4901234567818", "rejected", "Mismatch between saved overlay and proof subject.", "review archive", "artifact retained / route closed", "Rejected proof cannot unlock print.", "Correct the subject mismatch and re-run proof."));
        model.RouteRows.Add(new PropertyRowModel("Primary route", "service pdf-proof"));
        ...
        model.JobRows.Add(new JobRowModel("job-240416-018", null, "200-145-3", "JAN-LAB", "approved", "pdf-proof", "ready", "Lineage locked to proof-240416-018", "basic-50x30@v2", "4901234567894", "24", "lineage-7ba1a3", "No blocker. Route is open.", "Dispatch when the queue window is clear."));
        model.JobRows.Add(new JobRowModel("job-240416-017", null, "200-145-4", "JAN-LAB", "pending", "pdf-proof", "held", "Awaiting operator review", "basic-50x30@v2", "4901234567895", "24", "lineage-pending", "proof-240416-017 is still pending review.", "Hold until a reviewer approves or rejects the proof."));
        model.JobRows.Add(new JobRowModel("job-240416-019", null, "200-145-5", "JAN-LAB", "approved", "pdf-proof", "ready", "Local overlay saved and matched", "basic-50x30@v2", "4901234567818", "12", "lineage-45de90", "No blocker. Overlay and proof match.", "Dispatch after current ready jobs finish."));
        model.JobRows.Add(new JobRowModel("job-240416-020", null, "200-145-6", "JAN-LAB", "missing", "n/a", "blocked", "No proof artifact on record", "basic-50x30@v2", "4901234567800", "8", "none", "Missing proof artifact keeps route closed.", "Generate proof and obtain approval before queueing."));
        ...
        model.ControlSections.Add(new PropertySectionModel("Operator Actions", "The native shell now owns proof create/review while direct print dispatch stays in the service workflow.", new[] { new PropertyRowModel("Run proof", "native local path"), new PropertyRowModel("Proof review", "native local ledger"), new PropertyRowModel("Dispatch", "service-backed") }));
        ...
        model.MessageRows.Add(new MessageRowModel("Info", "proof", "Run Proof now creates a fresh pending proof from the selected subject row in the native shell."));
        model.MessageRows.Add(new MessageRowModel("Info", "route", "Current route is PDF-only until physical validation resumes."));
        model.StatusItems.Add(new StatusItemModel("Bridge", "connected", "service route checks responding", "OK", Brushes.ForestGreen));
    ...
    private static BatchJobsWorkspaceModel BuildBatchJobsWorkspace()
    {
        var model = new BatchJobsWorkspaceModel
        {
            ...
            FooterDetail = "shared batch snapshot is not loaded in seeded mode",
        ...
        model.ImportSessions.Add(new QueueItemModel("shared-batch-snapshot", "companion required", "seeded", "Seeded fallback keeps the Batch Jobs lane readable even when the shared snapshot is unavailable.", "windows-shell fallback", "shared snapshot", "No live snapshot is currently loaded.", "Refresh after admin-web publishes a batch snapshot."));
        model.ColumnRows.Add(new PropertyRowModel("Snapshot route", "shared batch snapshot"));
        model.ColumnRows.Add(new PropertyRowModel("Workflow", "apps/admin-web + service"));
        ...
        model.BatchRows.Add(new BatchRowModel("seeded-batch-ready", "basic-50x30@v2", "1 row", "ready", "Seeded example row for workstation layout review.", "1", "0", "shared snapshot", "Live row not loaded yet.", "Refresh after admin-web publishes a queue."));
        model.BatchRows.Add(new BatchRowModel("seeded-batch-failed", "proof-ticket@v1", "1 row", "failed", "Seeded blocker example so the focus pane keeps realistic copy in fallback mode.", "0", "1", "shared snapshot", "Live row not loaded yet.", "Handle the real retry from admin-web once the shared snapshot is available."));
        model.ActivityRows.Add(new ActivityRowModel("seeded", "batch", "Batch Jobs is waiting on the shared batch snapshot.", "watch"));
        model.ActivityRows.Add(new ActivityRowModel("scope", "guardrail", "Import, retry, and submit remain disabled in WPF.", "lock"));
        model.ControlSections.Add(new PropertySectionModel("Shared Snapshot Contract", "Batch Jobs now expects a shared snapshot instead of hardcoded queue authority.", new[] { new PropertyRowModel("Published by", "apps/admin-web"), new PropertyRowModel("Read by", "apps/windows-shell"), new PropertyRowModel("Mutation path", "apps/admin-web") }));
        model.ControlSections.Add(new PropertySectionModel("Seeded Fallback", "This fallback keeps the lane readable until the companion can load the real shared snapshot.", new[] { new PropertyRowModel("Use this lane for", "layout review and shell wording"), new PropertyRowModel("Do not use this lane for", "actual import or submit"), new PropertyRowModel("Refresh source", "companion service") }));
        model.MessageRows.Add(new MessageRowModel("Info", "batch", "Batch Jobs now expects a shared snapshot published by admin-web."));
        ...
        model.StatusItems.Add(new StatusItemModel("Workflow", "service-backed", "Batch mutation remains outside the native shell.", "LOCK", Brushes.Firebrick));
        model.StatusItems.Add(new StatusItemModel("Mode", "fallback", "Refresh once a queue snapshot exists.", "INFO", Brushes.SteelBlue));
